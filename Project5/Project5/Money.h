...
public:
    ...
    virtual void Otchet() {
        for (int i = 0; i < R.size(); ++i)
            cout << "День: " << R[i].getDay() << "\tНеделя: " << R[i].getWeek() << "\tМесяц: " << R[i].getMonth() << "\tТип: " << R[i].getType() << "\tКатегория: " << R[i].getCategory() << "\tПотрачено: " << R[i].getSpend() << endl;
    }
    //Сохранение отчета
    void SaveOtchet() {
        string filename;
    ...
    //Сортировка для рейтинга за категориями
